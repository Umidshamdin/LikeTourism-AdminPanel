...
function RoomImages() {
  ...
  const loadHotels = async () => {
    const results = await axios.get(
      `
      https://localhost:44363/api/HouseRoomImages/GetAll/${id}`
    );
    setRoomImages(results.data);
  };
    const deleteRoomImages = async (id) => {
      await axios.delete(`
      https://localhost:44363/api/HouseRoomImages/Delete/${id}`);
      loadHotels();
    };
  ...
  return (
    <div className="tables">
      <Link to="/createhouseroomimages" className="btn btn-success btn-fw link">
        Create Hotel Images
      </Link>
